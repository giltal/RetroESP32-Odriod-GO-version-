...
//{#pragma region Emulator and Directories
  ...
  int PROGRAMS[COUNT] = {1, 2, 2, 3, 3, 3, 4, 5, 6, 7, 8, 9, 10, 8, 9, 10};
  int LIMIT = 6;
...
//{#pragma region Helpers
  ...
  bool matches_rom_extension(const char *name, int step) {
    ...
    if (step == 15 && ext_eq(ext, "atr")) return true;
    return false;
  }
  ...
  int get_application (char* ext) {
    ...
    if(ext_eq(ext, "gb")) {application = 2;}
    if(ext_eq(ext, "gbc")) {application = 2;}
    ...
    if(ext_eq(ext, "a26")) {application = 5;}
    if(ext_eq(ext, "a78")) {application = 6;}
    if(ext_eq(ext, "lnx")) {application = 7;}
    if(ext_eq(ext, "pce")) {application = 8;}
    if(ext_eq(ext, "xex")) {application = 10;}
    if(ext_eq(ext, "atr")) {application = 10;}
    return application;
  }
