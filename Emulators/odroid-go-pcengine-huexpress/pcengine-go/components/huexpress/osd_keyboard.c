...
static int show_game_menu(void)
{
    ...
    /* Draw static parts: top border, title, spacer, bottom border */
    ...
    while (1)
    {
        ...
        if (cur != prev_cur) {
            ...
            for (i = 0; i < NUM_ITEMS; i++) {
                ...
                char buf[24];
                snprintf(buf, sizeof(buf), "%s%s", (i == cur) ? "> " : "  ", labels[i]);
                odroid_ui_draw_chars(MX, MY + (3 + i) * 8, MW, buf, fg, cbg);
            }
            prev_cur = cur;
        }
        ...
        if (last_key >= 0) {
            if (!js.values[last_key]) last_key = -1;
        } else {
...
osd_keyboard (void)
{
    ...
    /* MENU button: short press shows game menu, long press (>2s) saves & quits */
    if (ignoreMenuButton)
    {
    ...
    if (!ignoreMenuButton && previousJoystickState.values[ODROID_INPUT_MENU] && !joystick.values[ODROID_INPUT_MENU])
    {
        ...
        /* choice == 0: Continue - just return to game */
    }
    if ((!previousJoystickState.values[ODROID_INPUT_VOLUME] &&
        joystick.values[ODROID_INPUT_VOLUME]) || menu_restart)
    {
        menu_restart = odroid_ui_menu_ext(menu_restart, &menu_pceninge_init);
    }
