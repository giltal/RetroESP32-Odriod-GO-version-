...
{
  // Allocate single frame buffer — both pointers alias the same memory.
  // This saves 51KB of SPIRAM. Phosphor blending in main.cpp handles
  // flicker reduction using its own double-buffered SPIRAM copies.
  myCurrentFrameBuffer = new uInt8[160 * 320];
  myPreviousFrameBuffer = myCurrentFrameBuffer;
...
TIA::~TIA()
{
  // Single buffer — both pointers alias the same allocation
  delete[] myCurrentFrameBuffer;
  // myPreviousFrameBuffer is the same pointer, do NOT double-free
}
