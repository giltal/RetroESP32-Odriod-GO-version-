...
/* Paddle potentiometer raw ADC value — updated alongside joystick reads
 * in odroid_input_read_raw().  Now on GPIO15 / ADC2_CHANNEL_3.
 * -1 means not yet read. */
...
odroid_gamepad_state odroid_input_read_raw()
{
    ...
    /* Paddle ADC2 read moved to the main emulation loop (main.cpp)
     * to avoid SAR2 contention with the built-in DAC (BUILTIN_DAC_ENABLED=1).
     * The I2S DMA drives the DAC through the SENS/SAR2 block; calling
     * adc2_get_raw() here at 100 Hz caused ~25% frame-rate loss. */
...
void odroid_input_gamepad_init()
{
	...
	adc2_config_channel_atten(ADC2_CHANNEL_3, ADC_ATTEN_11db);  /* paddle pot on GPIO15 */
