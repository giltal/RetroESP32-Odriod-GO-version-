...
void app_main()
{
    ...
    // Safe mode: hold A during boot to bail out to launcher
    {
        ...
        if (bail.values[ODROID_INPUT_A]) {
            printf("prosystem-go: Button A held at boot — returning to launcher\n");
    ...
    if (!romfile || strlen(romfile) < 4)
    {
        ...
        printf("prosystem-go: No ROM path in NVS, entering file browser...\n");
        romfile = ChooseFile();
    }
    printf("%s: filename='%s'\n", __func__, romfile);
