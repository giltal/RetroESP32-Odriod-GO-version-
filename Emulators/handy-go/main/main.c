...
/* stub: planned debug function never implemented upstream */
static inline void odroid_ui_debug_enter_loop(void) {}
...
{
    /* Ensure the UI framebuffer is allocated (odroid_ui_init is not
       called by handy-go, so first MENU press would crash otherwise). */
    odroid_ui_init();

    /* Stop the video task so it cannot overwrite our menu overlay.
       The task renders frames from vidQueue and would race with our
       direct SPI writes, causing the menu to be invisible. */
    if (videoTaskIsRunning) {
        uint16_t* param = TASK_BREAK;
        xQueueSend(vidQueue, &param, portMAX_DELAY);
        while (videoTaskIsRunning) { vTaskDelay(1); }
    ...
    /* Menu geometry: 20 chars wide, 7 lines tall, centered on 320x240 screen */
    #define GMX  80      /* (320 - 20*8) / 2 */
    #define GMY  92      /* (240 - 7*8) / 2  */
    #define GMW  20      /* width in chars    */
    const uint16_t bg     = C_NAVY;
    const uint16_t title  = C_WHITE;
    const uint16_t normal = C_SILVER;
    const uint16_t hl     = C_YELLOW;

    int sel = 0;           /* 0=Continue, 1=Restart, 2=Quit */
    const char *items[3] = {
        "    > Continue      ",
        "    > Restart Game  ",
        "    > Quit to Menu  "
    };
    const char *items_n[3] = {
        "      Continue      ",
        "      Restart Game  ",
        "      Quit to Menu  "
    };
    ...
    /* draw box */
    odroid_ui_draw_chars(GMX, GMY + 0*8, GMW, "                    ", bg, bg);
    odroid_ui_draw_chars(GMX, GMY + 1*8, GMW, "    GAME  MENU      ", title, bg);
    odroid_ui_draw_chars(GMX, GMY + 2*8, GMW, "                    ", bg, bg);
    odroid_ui_draw_chars(GMX, GMY + 3*8, GMW, items[0],  hl,     bg);
    odroid_ui_draw_chars(GMX, GMY + 4*8, GMW, items_n[1], normal, bg);
    odroid_ui_draw_chars(GMX, GMY + 5*8, GMW, items_n[2], normal, bg);
    odroid_ui_draw_chars(GMX, GMY + 6*8, GMW, "                    ", bg, bg);
    ...
    while (1)
    {
        ...
        int new_sel = sel;
        if (js.values[ODROID_INPUT_UP]   && last_key != ODROID_INPUT_UP)   { new_sel = (sel > 0) ? sel - 1 : 2; }
        if (js.values[ODROID_INPUT_DOWN] && last_key != ODROID_INPUT_DOWN) { new_sel = (sel < 2) ? sel + 1 : 0; }

        if (new_sel != sel) {
            sel = new_sel;
            odroid_display_lock();
            for (int i = 0; i < 3; i++) {
                odroid_ui_draw_chars(GMX, GMY + (3+i)*8, GMW,
                    (i == sel) ? (char*)items[i] : (char*)items_n[i],
                    (i == sel) ? hl : normal, bg);
            }
            odroid_display_unlock();
        }
        ...
        if (js.values[ODROID_INPUT_A]) {
            ...
            break;
        } else if (js.values[ODROID_INPUT_B] || js.values[ODROID_INPUT_MENU]) {
            last_key = js.values[ODROID_INPUT_B] ? ODROID_INPUT_B : ODROID_INPUT_MENU;
            sel = 0;  /* Continue */
            break;
        }

        /* track held keys for debounce */
        last_key = -1;
        if (js.values[ODROID_INPUT_UP])   last_key = ODROID_INPUT_UP;
        if (js.values[ODROID_INPUT_DOWN]) last_key = ODROID_INPUT_DOWN;
    }
    ...
    /* Restart the video task so emulation display resumes */
    update_display_task();
...
void process_keys(odroid_gamepad_state *joystick)
{
        ...
        /* Show game menu on MENU release (no long-press DoMenuHome —
           frame-counting is unreliable with FRAME_SKIP) */
        if (!ignoreMenuButton && previousState.values[ODROID_INPUT_MENU] && !joystick->values[ODROID_INPUT_MENU])
        {
            ...
            /* choice == 0: Continue - return to game */
        }
...
void odroidgo_retro_init(void) {
	printf("%s - start\n", __func__);
        ...
        /* Try NVS ROM path first (set by launcher) */
        char *rc = odroid_settings_RomFilePath_get();
        if (rc && strlen(rc) > 4) {
            printf("NVS RomFile: %s\n", rc);
        } else {
            /* Fallback to file browser */
            if (rc) free(rc);
            odroid_display_unlock();
            rc = odroid_ui_choose_file("/sd/roms/lynx", "lnx");
            odroid_display_lock();
        }
        if (rc) {
           printf("File: %s\n", rc);
...
printf("lynx-handy (%s-%s).\n", COMPILEDATE, GITREV);
    ...
    // Safe mode: hold A during boot to bail out to launcher
    {
        ...
        if (bail.values[ODROID_INPUT_A]) {
            printf("handy-go: Button A held at boot — returning to launcher\n");
