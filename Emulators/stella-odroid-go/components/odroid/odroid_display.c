...
{
    ili9341_write_frame_rectangleLE(0, 0, image_splash.width, image_splash.height, (uint16_t*)image_splash.pixel_data);
...
void odroid_display_show_sderr(int errNum)
{
    switch(errNum)
    {
        case ODROID_SD_ERR_BADFILE:
            ili9341_write_frame_rectangleLE(0, 0, image_sd_card_unknown.width, image_sd_card_unknown.height, (uint16_t*)image_sd_card_unknown.pixel_data); // Bad File image
            break;

        case ODROID_SD_ERR_NOCARD:
            ili9341_write_frame_rectangleLE(0, 0, image_sd_card_alert.width, image_sd_card_alert.height, (uint16_t*)image_sd_card_alert.pixel_data); // No Card image
            break;
