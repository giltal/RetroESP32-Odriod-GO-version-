...
// Paddle potentiometer constants
...
void DrawPage(char** files, int fileCount, int currentItem)
{
	...
	else
	{
	    ...
	    for (int line = 0; line < ITEM_COUNT; ++line)
	    {
            ...
            size_t fileNameLength = dot ? (size_t)(dot - fileName) : strlen(fileName);
...
static const char* ChooseFile()
{
    ...
    UG_WindowSetTitleText(&window1, "SELECT ROM");
    UG_WindowSetTitleTextFont(&window1, &FONT_8X12);
...
void stella_step(odroid_gamepad_state* gamepad)
{
    ...
    // Paddle potentiometer: read ADC value from odroid_input and map to Stella axis.
    // Only inject axis for Paddle games — otherwise Joystick::update() interprets
    // the axis as directional input, moving the character in joystick games.
    if (console->controller(Controller::Left).type() == Controller::Paddles) {
        ...
        // Map ADC range to Stella axis: ADC_LO → +32767, ADC_HI → -32767
        int clamped = ema_adc;
        ...
        // Dead zone: only update when axis moves enough
...
/* ===================================================================
 * Save / Load state helpers (using Stella's Serializer)
 * =================================================================== */
...
static void build_save_path(const char *romfile)
{
    ...
    snprintf(save_path, sizeof(save_path), "/sd/odroid/data/a26/%s.sav", name);
    printf("Save path: %s\n", save_path);
}

static bool SaveState(void)
{
    if (!console || save_path[0] == '\0') return false;

    ensure_save_dir();

    // Use Stella's file-based Serializer (read/write mode)
    ...
    ser.reset();
    if (!console->save(ser)) {
        printf("ERROR: Console::save failed\n");
        return false;
    }

    printf("State saved OK to %s\n", save_path);
...
static bool LoadState(void)
{
    ...
    // Use Stella's file-based Serializer (readonly mode)
    string savefile(save_path);
    Serializer ser(savefile, true);
    if (!ser.isValid()) {
    ...
    ser.reset();
    if (!console->load(ser)) {
        printf("ERROR: Console::load failed\n");
        return false;
    }

    printf("State loaded OK from %s\n", save_path);
    return true;
}
...
static int show_game_menu()
{
    ...
    while (true)
    {
        ...
        // Draw border box
        UG_FillFrame(60, 30, 260, 210, C_NAVY);
        UG_DrawFrame(60, 30, 260, 210, C_WHITE);
        UG_DrawFrame(61, 31, 259, 209, C_WHITE);

        // Title
        UG_FontSelect(&FONT_10X16);
        UG_SetForecolor(C_WHITE);
        UG_SetBackcolor(C_NAVY);
        UG_PutString(105, 38, (char*)"GAME MENU");

        // Draw separator line
        UG_FillFrame(70, 58, 250, 59, C_WHITE);
...
extern "C" void app_main()
{
    ...
    // Safe mode: hold A during boot to bail out to launcher
    {
        ...
        if (bail.values[ODROID_INPUT_A]) {
            printf("stella-go: Button A held at boot — returning to launcher\n");
    ...
    /* Build save path from ROM filename */
    ...
    printf("DataSlot (startAction): %d\n", (int)startAction);
    if (startAction == 1 && save_path[0] != '\0') {
        if (LoadState()) {
