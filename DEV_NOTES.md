...
## v3.15 — ZX Spectrum Kempston Joystick Fix

### Problem

The gamepad was not mapped to the Kempston joystick in the ZX Spectrum emulator. Games that use Kempston input (the most common joystick type on the ZX Spectrum) did not respond to the gamepad. The D-pad and buttons were only mapped to Spectrum keyboard keys via the `kbstate[]` array, which goes through the keyboard matrix — not the Kempston port.

### Root Cause

In `vgakey.c`, `keyboard_update()` reads the Odroid Go gamepad and sets `kbstate[map[b_up]]` etc., which maps to Spectrum keyboard scancodes (default: keys 4/5/6/7/8/9 — the "Cursor" joystick keys). The Kempston joystick is a separate I/O port (`0x1F`) read by the Z80 via the `IN` instruction, and its value comes from `z80_inports[0x1F]`. This port was never written by the gamepad handler — it was only set by `spkb_refresh()` in `spkey.c` when processing PC keyboard-style key events (arrow keys mapped to Kempston via `joycurs[]`), which never fires on the Odroid Go since there's no PC keyboard.

### Fix

**Files modified:**
- `Emulators/odroid-go-spectrum-emulator/components/spectrum/vgakey.c`
- `Components/odroid-go-spectrum-emulator/components/spectrum/vgakey.c`

**Changes (both files identical):**

1. **Added `#include "z80.h"`** — provides access to `z80_inports[]` (the Z80 port I/O array)

2. **Clear Kempston port each frame** — `z80_inports[0x1F] = 0` at the start of `keyboard_update()`, alongside the `kbstate[]` clear loop. Prevents stale values when in virtual keyboard mode.

3. **Write Kempston port from gamepad state** — In the non-keyboard branch (when `!keyboard`), builds a Kempston byte from the D-pad and A/B buttons using the standard Kempston bit layout:
...
| 2 | `0x04` | Down |
| 3 | `0x08` | Up |
| 4 | `0x10` | Fire (A or B button) |

The Kempston byte is written to `z80_inports[0x1F]` before the existing keyboard key mapping, so both Kempston-aware and keyboard-aware games work.
